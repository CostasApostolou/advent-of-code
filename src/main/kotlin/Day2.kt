...
private fun solvePart1(): Int =
    input
        .lines()
        .map { line ->
            val gameId = line.substringBefore(":").parseGameId()
            val games = line.substringAfter(":").split(";").map { game -> game.toGame() }
            gameId to games
        }
        .filter { (_, games) -> games.all { it.isValid() } }
        .sumOf { it.first }
...
private fun solvePart2(): Int =
    input
        ...
        .sumOf { (_, games) -> games.toMinimumSet().power }
