fun main() {
    println(solvePart1(sampleInput1.lines()))
    println(solvePart1(input.lines()))
    println(solvePart2(sampleInput2.lines()))
...
private fun solvePart2(lines: List<String>): Int =
    lines.map { it.translate().toTwoDigitNum() }.fold(0) { acc, i -> acc + i }

private fun String.translate(): String =
    regex.findAll(this).map { it.groupValues.last().toNum() }.joinToString("")
...
private fun String.toTwoDigitNum(): Int {
    val intermediate = mapNotNull { it.digitToIntOrNull() }
    val firstDigit = intermediate.first()
    val secondDigit = if (intermediate.size > 1) intermediate.last() else firstDigit
...
fivetczxxvjrrqfive1sevennvj6one3""".trimIndent()