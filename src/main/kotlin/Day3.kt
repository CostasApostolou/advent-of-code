fun main() {
    ...
    val (parts, symbols) = parseInputForPart1(input.lines())
    parts.forEach { println(it) }
    println()
    symbols.forEach { println(it) }
    println()
    println(solvePart1(parts, symbols))
    println(solvePart2(parts, symbols))
}

private fun parseInputForPart1(lines: List<String>): Pair<List<PartNumber>, List<Symbol>> {
    ...
    return lines
        .flatMapIndexed { row, line ->
            ...
            while (column < line.length) {
                if (line[column].isDigit()) {
                    val id = line.substring(column).toCharArray().takeWhile { it.isDigit() }.joinToString("")
                    ...
                    if (line[column] != '.') {
                        symbols.add(Symbol(line[column], Coordinate(row, column)))
                    }
                    column++
                }
            }
        ...
        .filter { it.isValid(symbols.map { it.position }) }
        .let { it to symbols }
}

private fun solvePart1(potentialParts: List<PartNumber>, symbols: List<Symbol>): Int =
    potentialParts
        .filter { it.isValid(symbols.map { it.position }) }
        .sumOf { it.number }

private fun solvePart2(potentialParts: List<PartNumber>, symbols: List<Symbol>): Long {
    val parts = potentialParts
        .filter { it.isValid(symbols.map { it.position }) }
    val gears = symbols.filter { it.id == '*' }

    return gears
        .mapNotNull { it.position.collectNeighbours(parts).takeIf { it.size == 2 } }
        .sumOf { it.productOf() }
}

private fun List<PartNumber>.productOf(): Long =
    fold(1){ acc, part -> acc * part.number}

private data class Symbol(val id: Char, val position: Coordinate)
...
private data class Coordinate(val x: Int, val y: Int) {
    ...
    fun collectNeighbours(candidates: List<PartNumber>): List<PartNumber> =
        candidates.filter { partNumber -> partNumber.coordinates.any { it.isNeighbour(this) } }
}

private data class PartNumber(val number: Int, val coordinates: List<Coordinate>) {
    fun isValid(symbols: List<Coordinate>): Boolean =
        coordinates.any { part -> symbols.any { it.isNeighbour(part) } }
}
